#pragma once

#include <Arduino.h>
#include "scan_config.h"
#include "sensors.h"
#include "filament_data.h"

// ============================================================
// Filla IQ — Scan Station TFT Display (ST7789V3 240x280 SPI)
// ============================================================

class Display {
public:
    void begin();
    void update(ScanState state, float weight, bool stable,
                const char* nfcUid,
                const FilamentInfo* filament,
                const DistanceData* distance,
                const ColorData* color);
    void showMessage(const char* line1, const char* line2 = nullptr);
    void showQrCode(const char* data, const char* label = nullptr);

private:
    bool _ready;
    ScanState _lastState;
    unsigned long _lastUpdate;
    bool _forceRedraw;

    void drawHeader(const char* title, uint16_t titleColor);
    void drawIdle();
    void drawUnknown(float weight, bool stable, const DistanceData* dist, const ColorData* color);
    void drawSpool(float weight, bool stable, const FilamentInfo& fi, const DistanceData* dist);
    void drawSpoolIcon(int cx, int cy, int w, int h, uint16_t fillColor);
};

extern Display display;
